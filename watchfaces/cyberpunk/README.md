...
Os campos de texto não rolam quando um valor ultrapassa sua largura. Se a previsão do tempo não estiver sincronizada, a área de clima mostra `--°` em vez de inventar um valor.
...
`Weather` e `Sleep` não expõem `onChange`, então são relidos no tique de minuto do relógio: sem isso a temperatura ficaria travada em `--°` quando a sincronização do app Zepp termina depois que o mostrador já carregou.
